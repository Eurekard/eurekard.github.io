...
import { AnonResponse } from '../../types';
import { MessageSquareOff, Archive, Trash2, Send, Filter, CheckCircle2, Circle } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';
import { zhTW } from 'date-fns/locale';
import { cn } from '../../lib/utils';

export default function ResponsesView({ cardId }: { cardId: string }) {
        ...
        <div>
          <h2 className="text-2xl font-display font-bold text-chocolate flex items-center gap-3">
            ...
            <span className="text-sm font-bold bg-cat-blue/10 text-cat-blue px-3 py-1 rounded-full">
              {responses.filter(r => r.status === 'unread').length} 未讀
            </span>
          </h2>
          <p className="text-chocolate/60 text-sm">在這裡查看訪客留給你的秘密悄悄話</p>
        </div>
